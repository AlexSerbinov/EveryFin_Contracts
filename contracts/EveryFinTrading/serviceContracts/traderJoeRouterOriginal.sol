...
//         if (_pairBinSteps.length == 0 || _pairBinSteps.length + 1 != _tokenPath.length) revert LBRouter__LengthsMismatch();
//         _;
//     }
...
//                 (uint256 _amountInToBin, uint256 _amountOutOfBin, FeeHelper.FeesDistribution memory _fees) = _bin.getAmounts(_fp, _activeId, _swapForY, _amountIn);
...
//     function addLiquidity(LiquidityParameters calldata _liquidityParameters) external override returns (uint256[] memory depositIds, uint256[] memory liquidityMinted) {
//         ILBPair _LBPair = _getLBPairInformation(_liquidityParameters.tokenX, _liquidityParameters.tokenY, _liquidityParameters.binStep);
...
//     /// @return depositIds Bin ids where the liquidity was actually deposited
//     /// @return liquidityMinted Amounts of LBToken minted for each bin
//     function addLiquidityAVAX(LiquidityParameters calldata _liquidityParameters) external payable override returns (uint256[] memory depositIds, uint256[] memory liquidityMinted) {
//         ILBPair _LBPair = _getLBPairInformation(_liquidityParameters.tokenX, _liquidityParameters.tokenY, _liquidityParameters.binStep);
//         if (_liquidityParameters.tokenX != _LBPair.tokenX()) revert LBRouter__WrongTokenOrder();
...
//             (uint256 _amountX, uint256 _amountY) = _removeLiquidity(_LBPair, _amountTokenMin, _amountAVAXMin, _ids, _amounts, address(this));
...
//         uint256 _deadline
//     ) external override ensure(_deadline) verifyInputs(_pairBinSteps, _tokenPath) returns (uint256 amountOut) {
//         if (_tokenPath[_pairBinSteps.length] != IERC20(wavax)) revert LBRouter__InvalidTokenPath(address(_tokenPath[_pairBinSteps.length]));
...
//         address _to,
//         uint256 _deadline
//     ) external payable override ensure(_deadline) verifyInputs(_pairBinSteps, _tokenPath) returns (uint256[] memory amountsIn) {
//         if (_tokenPath[0] != IERC20(wavax)) revert LBRouter__InvalidTokenPath(address(_tokenPath[0]));
...
//     {
//         unchecked {
//             if (_liq.deltaIds.length != _liq.distributionX.length && _liq.deltaIds.length != _liq.distributionY.length) revert LBRouter__LengthsMismatch();

//             if (_liq.activeIdDesired > type(uint24).max || _liq.idSlippage > type(uint24).max) revert LBRouter__IdDesiredOverflows(_liq.activeIdDesired, _liq.idSlippage);

//             (, , uint256 _activeId) = _LBPair.getReservesAndId();
//             if (_liq.activeIdDesired + _liq.idSlippage < _activeId || _activeId + _liq.idSlippage < _liq.activeIdDesired)
...
//             (_amountXAdded, _amountYAdded, liquidityMinted) = _LBPair.mint(depositIds, _liq.distributionX, _liq.distributionY, _liq.to);
...
//         if (amountX < _amountXMin || amountY < _amountYMin) revert LBRouter__AmountSlippageCaught(_amountXMin, amountX, _amountYMin, amountY);
//     }
...
//         if (address(_LBPair) == address(0)) revert LBRouter__PairNotCreated(address(_tokenX), address(_tokenY), _binStep);
//         return _LBPair;
//     }
...
//     function _getPairs(uint256[] memory _pairBinSteps, IERC20[] memory _tokenPath) private view returns (address[] memory pairs) {
